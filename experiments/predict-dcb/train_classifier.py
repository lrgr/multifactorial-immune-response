...
from models import MODEL_NAMES, init_model, RF, EN, LR, FEATURE_CLASSES
...
parser.add_argument('-m', '--model', type=str, required=True, choices=[LR])
...
# Assess and record held-out predictions
...
json_output.update({
    ...
    "AUROC": roc_auc_score(sub_y, sub_preds),
    "AUPR": average_precision_score(sub_y, sub_preds)
})
print(json_output)
...
elif args.model == EN or args.model == LR:
    if len(model.named_steps['estimator'].coef_.shape) == 2:
        coef = model.named_steps['estimator'].coef_[0]
    else:
        coef = model.named_steps['estimator'].coef_
    variable_scores = coef * X.loc[:,training_cols].fillna(X.loc[:,training_cols].median()).std()
else:
...
logger.info('Feature importances' if args.model == RF else 'Learned coefficients')
logger.info('-' * len(rows[0]))
for row in rows:
    logger.info(row)
logger.info('')
